...
from response_correlation_clustering import HdbscanCellClusterer, StimulusCrossCorrelation
import os
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE" #otherwise there are errors with parallel processing?
...
full_traces_df, cell_to_age = pickle_load_object(
    r".\data\3to5dpf\2026-06-24_3to5dpf_trace_data_reliability_filtered.pkl")
...
regressors_omr = np.load(r"C:\Users\ag-bahl\Desktop\Max\master_thesis\Flo_kmeans_regressors.npy") * 100
motion_regressors = regressors_omr[:-1, None, :]  # ['motion_onset', 'motion_integrator_1', 'slow_motion_integrator']
correlation_dfs = []
total_cells = {a:0 for a in ['3dpf', '4dpf', '5dpf']}
for (age, region), grouped_df in full_traces_df.groupby(level=('age', 'region')):
...
# %% plotting luminance correlated cells (Fig 7 A)
...
# %% plotting the motion correlated cells (Fig 7 B)
...
# %% plot correlations as box plots (Fig 6)
...
# %% active cell fractions (Fig 5)
cell_numbers = pickle_load_object(r".\data\3to5dpf\2026-06-24-cell_numbers_3to5dpf.pkl")
...
# %% load masks and plot areas that change (Fig 8)
...
#%% plot traces of most changed regions (Fig 9 A)
...
#%% correlation to motion regressors boxplots (Fig 9 B)
correlation_dfs = []
...
#%% Clustering with 1080 parameter sets
...
for age in ages:
    ...
    for dist_met,min_sample,min_clust,max_clust,sel_met in product(
        ...
        if i % 10 == 0:
            pickle_save_object(results, fr'.\data\{today}_clustering_1080_parameters_results.pkl')
            get_reusable_executor().shutdown(wait=True, kill_workers=True)
        gc.collect()

#%% plot results of different parameters (Fig 13)
clustering_results = pickle_load_object(r'.\data\2026-07-08_clustering_1080_parameters_results.pkl')
...
#%% create some meaningful clusters for 5, 4, and 3dpf (5) (Fig 10-12)
cluster_class = HdbscanCellClusterer(
        full_traces_df,
        ordered_stimuli=['lumi_ipsi_dots_off', 'lumi_contra_dots_off', 'lumi_off_dots_ipsi', 'lumi_off_dots_contra'],
        n_jobs=16,
